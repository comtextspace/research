...
Электронная библиотека — сайт на котором можно скачать книги. Похожих электронных библиотек очень много, рассматривать их все не имеет смысла. Далее обзор нескольких из них, чтобы получить о них общее понимание.
...
http://lib.ru/COMPULIB/

## ilibrary.ru

[Интернет-библиотека Алексея Комарова](https://ilibrary.ru) существует с 1996 года. Сначала в виде журнала с авторскими комментарии, затем в виде сборника оригинальных текстов. Библиотека ориентированна на русскую классику 19 и начала 20 века. [Принципы библиотеки](https://ilibrary.ru/aboutilibrary/):
...
На странице автора находится краткое описание автора и список фильтров для произведений.
...
Для некоторых авторов есть раздел с произведениями об авторе и раздел со ссылками на произведения автора в других библиотеках. Не все ссылки работают.
...
Библиотека наполняется, последние добавления отображаются на главной странице.

![](/images/textreview/textreview-online-lib/textreview-ilibrary-ru-07.png)

На странице произведения указана информация о произведение и оглавление.
...
Внизу страницы произведений указана информация о копирайте

> © Это произведение перешло в общественное достояние. Произведение написано автором, умершим более семидесяти лет назад, и опубликовано прижизненно, либо посмертно, но с момента публикации также прошло более семидесяти лет. Оно может свободно использоваться любым лицом без чьего-либо согласия или разрешения и без выплаты авторского вознаграждения.

Скачивать книгу нельзя, только читать онлайн. Через оглавление можно перейти у нужному разделу или части книги. На каждой странице находятся ссылки для перехода к следующему и предыдущему разделу.
...
Поиск работает по названием произведений и именам авторов. Результаты отображаются по точному совпадению слова без учета регистра.
...
В некоторых книгах кроме описания указаны популярные фрагменты.

![](/images/textreview/textreview-online-lib/textreview-ilibrary-ru-15.png)

Они открываются как обычный раздел книги, но основная часть выделена цветом текста.
...
Скорее всего бекенд библиотеки написан на Perl, для фронтенда используются свои стили и чисты JavaScript. Исходный формат хранения текстов не известен. Доступа к оригинальному представлению книги нет, а только к веб представлению.

## traumlibrary.ru

[Библиотека русской и советской классики](https://traumlibrary.ru) — библиотека основанная на fb2-формате.
...
На этой же странице указано отношение к копирайту.
...
Навигация представлена в виде каталога по авторам.

![](/images/textreview/textreview-online-lib/textreview-traumlibrary-ru-03.png)

Отдельно есть группировка авторов по историческому периоду.
...
Один автор может находится в нескольких рубриках.

На страницах авторов указаны все книги в которых присутствуют произведения автора и отдельно список произведений и сборников.
...
Слева вверху указан версия книги «версия: 2.21» она же присутствует в fb2-файле.

![](/images/textreview/textreview-online-lib/textreview-traumlibrary-ru-14.png)

В этой есть разные виды примечаний: комментарии и примечания. Вероятно, таким образом сохранена структура печатного издания. Ниже ссылка на комментарий.
...
Примечания выглядят обычным способом.
...
Поиск формирует ссылку `https://cse.google.ru/cse?cx=partner-pub-5232438239599560:3411500519&q=белый` и ведет в Гугл.

![](/images/textreview/textreview-online-lib/textreview-traumlibrary-ru-23.png)

Технологический стек сайта не известен, возможно используется самописный генератор, на выходе получается статический сайт.

## RoyalLib.com

В футере сайта библиотеки [RoyalLib.com](https://royallib.com) указаны года её работы 2010-2025.

На сайте есть три каталога по наименованию: по авторам, по книгам и по сериям.
...
Слева находится каталог по жанрам. На отдельной странице находятся подразделы для каждого жанра.

![](/images/textreview/textreview-online-lib/textreview-royallib-com-04.png)

При переходе в подраздел отображается список книг в алфавитном порядке. Так как автор и год не указаны, то некоторые названия выглядят как дубли.
...
Пример цитат из раздела «цитаты о цитатах» заставляет предположить, что группировка по темам не соблюдается.

![](/images/textreview/textreview-online-lib/textreview-royallib-com-08.png)

Поиск работает по именам авторов, названиям книг и серий. Судя по всему поиск работает выдает все варианты нахождения подстроки без учёта регистра. 

![](/images/textreview/textreview-online-lib/textreview-royallib-com-09.png)

На странице автора находится список книг с группировкой по жанру.

![](/images/textreview/textreview-online-lib/textreview-royallib-com-10.png)

Каждая книга содержит небольшое описание и ссылки на книгу в нескольких форматах.

![](/images/textreview/textreview-online-lib/textreview-royallib-com-11.png)

Доступные форматы: `FB2`, `EPUB`, `DOC`, `HTML`, `RTF`, `TXT`. Все форматы, кроме `EPUB`, скачиваются в ZIP-архиве, с названием, вероятно в кодировке `windows-1251`, что осложняет использование на Linux.

![](/images/textreview/textreview-online-lib/textreview-royallib-com-12.png)

Форматы `TXT` и `HTML` тоже в кодировке `windows-1251`.

В начале и в конце книга в каждом формате содержит набор ссылок на библиотеку.

![](/images/textreview/textreview-online-lib/textreview-royallib-com-13.png)

Судя по всему, все форматы генерируются из `FB2`. Например, в книге Г. Плеханова «О роли личности в истории» в FB2-файле указан источник книги `https://www.esperanto.mv.ru/Marksismo/Pershist/pershist.html`, а программа через которую сделана книга `fb2edit 0.0.9`. Для книги «Приключения Чиполлино» указан источник `lib.aldebaran.ru`, а программа `FB Tools, FB Writer v1.1`.

Качество вычитки определяется источником книги. В самой библиотеке корректировка и исправление, вероятно, не производится.

Форма `TXT` получается приемлемый, но плохочитаемый. Сноски находятся внизу, в тексте отображаются следующим образом: `#red21: #n24.`.

Книгу можно читать на сайте. Доступны настройки размера и гарнитуры шрифта, а так же цвета текста и фона. Позиция чтения сохраняется, вероятно, в cookie, поэтому после закрытия или обновления страницы в браузере книга откроется в том же месте.

![](/images/textreview/textreview-online-lib/textreview-royallib-com-14.png)

Левое меню позволяет установить закладку. Список закладок будет отображаться на главной странице. Закладки так же сохраняются в браузере. Этот функционал не требует регистрации.
...
На [отдельной странице](https://royallib.com/copyright.html) находится обращение к владельцам авторских прав:
...
В случае если книга недоступна на странице книги будет следующий текст.

![](/images/textreview/textreview-online-lib/textreview-royallib-com-17.png)

На сайте библиотеки есть страница с распространенными вопросами, содержащая два вопроса:
...
Регистрация на сайте не даёт новых существенных возможностей и только упрощает добавление комментариев.

## Заключение

Электронные библиотеки похожи друг на друга. Основную часть сайта составляет каталог и сами книги. Каталоги различаются но состоят из разных комбинаций имён авторов, названий книг и произведений.

В библиотеках может быть возможность скачивать книги или только читать онлайн. Если есть скачивание то могут различаться форматы, но чаще всего присутствует fb2.

Технологические стеки библиотек существенно различаются, чаще всего это самописные решения. Исходного кода сайтов нет в открытом доступе.

Какие-то библиотеки представляют просто склад книг (RoyalLib.com), другие же направлены так же на вычитку и улучшение текста. В обоих случаях обмен исправлениями, вероятно, не производится. На выходе получается множество версий одного текста. Затраты труда дублируются.

В некоторых библиотеках результаты вычитки можно считать свободно распространяемыми, например, в случае библиотек основанных на fb2. Свободное распространение fb2 файла, позволяет конвертировать и дорабатывать текст. В других библиотеках открыто только веб-представление, а оно неудобно для дальнейшей работы с текстом.
