...
Текущая версия: 2rc2.0.0. (версия в разработке)
...
Не обязательное. Определяет жанр. Список жанров из [документации к формату ФБ2](http://www.fictionbook.org/index.php/Жанры_FictionBook_2.1).

Можно указать один жанр:

```yaml
genre: sci_history
```

Или несколько жанров
...
Для добавления стихотворений используйте вертикальную черту (`|`) в начале строки.
...
В книгах номера страниц размещаются внизу или вверху края страницы. При разметке размещайте номера страниц в начале страницы — до её содержимого. Такой подход обеспечивает предсказуемое позиционирование и упрощает создание точных ссылок.  
...
Конструкция `comment` должна вырезаться средствами отображения книг на этапе конвертации.
...
Для корректного экспорта книг в [формат ФБ2](http://www.fictionbook.org/index.php/Описание_формата_FB2_от_Sclex) обязательны следующие поля метаинформации:
