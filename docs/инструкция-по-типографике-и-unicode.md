...
```
Версия документа: 0.1 (03.09.2025)
```
...
Дальнейшие инструкции по вводу символов ориентированы на Linux. Для Windows схожие возможности можно реализовать с помощью специальных раскладок, таких как [Strata](https://way23.ru/раскладка-клавиатуры-strata.html) или [Типографской раскладки Ильи Бирмана](https://way23.ru/установка-типографской-раскладки-на-wind.html).

В дистрибутивах Linux раскладку можно настроить без установки дополнительных программ. Ниже — несколько инструкций для ParrotOS:
...
Для особенно сложных случаев поможет утилита [Espanso](https://way23.ru/espanso-ввод-спецсимволов.html).

Также можно найти множество других инструкций, например:
...
Короткое тире используется только в диапазонах: `20–24 стр.`, `20.04.2025–24.04.2025`. В русском языке нет знака «короткое тире», а только дефис и тире, поэтому использование короткого тире остаётся на усмотрение автора или издательства.

Минус применяется исключительно в математических формулах.

Длинное тире — знак препинания. Дефис используется в составных словах.
...
В исходных текстах должны использоваться только обычные пробелы ` ` ([Space](https://symbl.cc/en/0020/)). Остальные виды пробелов должны подставляться автоматически типографскими инструментами.
...
Основные — кавычки «ёлочки». При необходимости вложенных кавычек используются «лапки». Например: «он прочитал „Капитал“ за неделю».

Марровские кавычки применяются только в специализированной лингвистической литературе, например в словарях. В обычных текстах их использовать не следует.
...
Многоточие следует использовать вместо трёх точек подряд (`...`).
...
Символ ударения ([Combining Acute Accent](https://symbl.cc/en/0301/)) добавляется к буквам. Гласные русского языка с ударением:
...
Эти символы — комбинации кириллических букв и знака ударения. Их нельзя путать с латинскими аналогами. Например, `é` ([Latin Small Letter E with Acute](https://symbl.cc/en/00E9/)) — это не `е́` ([Cyrillic Small Letter Ie](https://symbl.cc/en/0435/) в комбинации с [Combining Acute Accent](https://symbl.cc/en/0301/)).
...
В качестве апострофа используется символ `’` ([Right Single Quotation Mark](https://symbl.cc/en/2019/)). Он же применяется как закрывающая английская кавычка (марровские кавычки).

Символ `’` нужно отличать от `'` ([Apostrophe](https://symbl.cc/en/0027/)). В русскоязычной типографике используется только первый.

Апостроф применяется при передаче иностранных фамилий с начальными `Д` и `О`:
...
В Unicode есть символы для надстрочных и подстрочных индексов. Их можно использовать в химических и математических формулах, а также в других случаях, где требуются индексы.
...
Также доступны математические операторы:
...
Все символы можно посмотреть в коллекции [Superscript and Subscript Numbers](https://symbl.cc/en/collections/superscript-and-subscript-numbers/).

## Дроби

В Unicode есть 19 символов для часто употребляемых дробей:
...
Остальные дроби создаются с помощью надстрочных и подстрочных индексов и следующих символов:
...
Обратите внимание: `⁄` ([Fraction Slash](https://symbl.cc/en/2044/)) — это не `/` ([Solidus](https://symbl.cc/en/002F/)).
...
Штрих используется в разных контекстах: угловые минуты и секунды в геометрии, производные в анализе, обозначение различных переменных через одну букву с разным числом штрихов.

Символ `′` ([Prime](https://symbl.cc/en/2032/)) нужно отличать от `'` ([Apostrophe](https://symbl.cc/en/0027/)), `‘` ([Left Single Quotation Mark](https://symbl.cc/en/2018/)), `’` ([Right Single Quotation Mark](https://symbl.cc/en/2019/)) и `´` ([Acute Accent](https://symbl.cc/en/00B4/)). Аналогично `″` ([Double Prime](https://symbl.cc/en/2033/)) не следует путать с `"` ([Quotation Mark](https://symbl.cc/en/0022/)).
...
* `+` — плюс ([Plus Sign](https://symbl.cc/en/002B/))
* `−` — минус ([Minus Sign](https://symbl.cc/en/2212/))
* `±` — плюс-минус ([Plus minus symbol](https://symbl.cc/en/00B1/))
* `×` — символ умножения ([Multiplication Sign](https://symbl.cc/en/00D7/))
* `÷` — символ деления ([Division Sign](https://symbl.cc/en/00F7/))
...
Особый случай — символ мягкого переноса ([Soft Hyphen](https://symbl.cc/en/00AD/)). Его не следует использовать в исходных текстах: он предназначен для автоматической расстановки переносов и должен добавляться типографскими системами, а не редакторами.

### Дополнительно

* [Алгоритм Ляна-Кнута для расстановки мягких переносов](https://habr.com/ru/post/138088/)

## Чего нехватает

* С учётом частых цитат и названий на других языках, полезно было бы добавить разделы по типографике для немецкого, английского и испанского языков.
* Кроме цифровых, в Unicode присутствуют также буквенные надстрочные и подстрочные символы.
